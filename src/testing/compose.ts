// This script is only loaded in thunderbird-testing builds (check
// manifest.json). It lets selenium know that Firenvim is ready to receive
// events by connecting to the coverage server through a websocket. Once
// connected, it can decide to push coverage information
import { makeRequest, makeRequestHandler } from "./rpc";

// Of course we have to ignore the case where coverage data doesn't exist.
...
function createSocket(): Promise<WebSocket> {
    ...
    socket.addEventListener('message', makeRequestHandler(socket, "compose", coverageData));
    return new Promise(resolve => socket.addEventListener("open", () => resolve(socket)));
}

import { isReady } from "../compose";
